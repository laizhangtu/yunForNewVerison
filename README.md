...
2. 目前新学期云运动3.5.4版本有issue反映代跑存在问题，最后结果不达标，不过通讯和中间点的轨迹都没有被检测到异常。

3. 遇到问题建议手机点开不达标记录，会明确的说是什么原因不达标。
      - 如果是时间原因或者简单的数据问题，改一改就行，直接编辑tasklist改成正常的值，时间的话只能等。
      - 如果是轨迹问题或者关键点问题，可能新学期猜的点坐标和范围出现了变换，建议不要使用自带的tasklist，自己跑一遍通过的来打表。
      - 如果是更为复杂原因，手机上没显示完全，可以post一下历史记录看看具体的原因字符串，还是看不出来问题就把json文件(历史记录的post和你打的表的json)以及log输出都通过issue提供，我有空时候会试着解决它。
